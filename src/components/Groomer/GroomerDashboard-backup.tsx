...
const GroomerDashboard: React.FC = () => {
  ...
  const getServiceName = (services: any) => {
    if (Array.isArray(services) && services.length > 0) {
      return services[0].name || services[0].service || 'Service';
    }
    return services?.name || services?.service || 'Service';
  };
  ...
  const getPetInfo = (client: any) => {
    if (!client.pets) return { name: 'Pet', breed: 'Unknown', species: 'dog' };
    
    if (Array.isArray(client.pets) && client.pets.length > 0) {
    ...
    return {
      name: client.pets.name || 'Pet',
      breed: client.pets.breed || 'Unknown', 
      species: client.pets.species || 'dog'
    };
  };
