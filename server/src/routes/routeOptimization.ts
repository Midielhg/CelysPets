...
router.post('/optimize', auth, async (req, res) => {
  try {
    const { startLocation, appointments }: RouteOptimizationRequest = req.body;
...
async function optimizeRouteSimple(startLocation: string, appointments: AppointmentLocation[]) {
...
async function optimizeRouteWithGoogleMaps(startLocation: string, appointments: AppointmentLocation[]) {
...
function calculateFuelCost(totalDistance: number): number {
...
async function getDistanceMatrix(locations: string[], apiKey: string) {
  try {
    const origins = locations.join('|');
    const destinations = locations.join('|');
    
    const response = await fetch(
      `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${encodeURIComponent(origins)}&destinations=${encodeURIComponent(destinations)}&key=${apiKey}&units=metric`
    );
...
function createMockDistanceMatrix(size: number) {
  const matrix = [];
...
function solveTSP(distanceMatrix: any[][], appointments: AppointmentLocation[]): number[] {
...
router.get('/suggestions/:date', auth, async (req, res) => {
  try {
